...
def reordenar_columnas(columnas):
    ...
    y_base = 220  # Y inicial para todas las columnas
    for columna in columnas:
        y = y_base
        for carta in columna:
            carta["rect"].y = y
            y += 35  # Espacio vertical entre cartas
...
def manejar_eventos(columnas, pila_descubierta, mazo_restante, huecos, boton_reiniciar, boton_menu, pantalla, fuente):
    ...
    for evento in pygame.event.get():
        if evento.type == pygame.QUIT:
            ...
            if procesar_mazo(x, y, mazo_restante, pila_descubierta):
                return

            # Si había una carta seleccionada, intenta soltarla en algún lugar
            if procesar_soltar(x, y, columnas, pila_descubierta, huecos):
                return
...
def procesar_mazo(x, y, mazo_restante, pila_descubierta):
    ...
    if mazo_rect.collidepoint(x, y):
        ...
        if mazo_restante:
            # Toma una carta del mazo y la muestra
            carta = mazo_restante.pop(0)
            carta["visible"] = True
            carta["rect"] = pygame.Rect(150, 50, ANCHO_CARTA, ALTO_CARTA)
            pila_descubierta.append(carta)
            reproducir_sonido("mover")

        elif pila_descubierta:
            # Si ya no hay cartas, se reinicia el mazo
            for carta in pila_descubierta:
                carta["visible"] = False
                carta["rect"] = pygame.Rect(X_MAZO, Y_MAZO, ANCHO_CARTA, ALTO_CARTA)
...
def procesar_soltar(x, y, columnas, pila_descubierta, huecos):
    ...
    if soltar_en_huecos(carta, grupo, x, y, huecos, pila_descubierta, columnas):
        return True

    if soltar_en_columnas(carta, grupo, x, y, columnas, pila_descubierta):
        return True
...
def soltar_en_huecos(carta, grupo, x, y, huecos, pila_descubierta, columnas):

    # No se puede soltar un grupo de varias cartas en los huecos
    if len(grupo) > 1:
        return False
...
def soltar_en_columnas(carta, grupo, x, y, columnas, pila_descubierta):
